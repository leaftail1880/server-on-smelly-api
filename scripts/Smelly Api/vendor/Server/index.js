...
world.events.entityHurt.subscribe((data) => {
  if (
    data.cause != "entity_attack" &&
    data.cause != "fire" &&
    data.cause != "fire_tick" &&
...
world.events.beforeItemUse.subscribe((data) => {
  if (data.item.id == "we:tool") {
    let item = data.item;
    let lore = item.getLore();
    if (lore && lore[0] == "Particle") {
      if (data.source.isSneaking) {
        if (lore[2] - 1 >= 0) {
          lore[1] = P[lore[2] - 1] ?? lore[1];
          lore[2] = String(lore[2] - 1);
          item.setLore(lore);
          data.source
