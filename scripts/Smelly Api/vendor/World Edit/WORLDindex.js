...
//SA.Build.chat.broadcast('§9♦ §fСкрипты были перезагружены.')
...
SA.Utilities.time.setTickInterval(() => {
  for (const p of world.getPlayers()) {
    if (SA.Build.entity.getHeldItem(p)?.id == "we:s" || (po.Q('mobile', p) && p.hasTag('using_item') && SA.Build.entity.getHeldItem(p)?.id == "we:s")) {
      const i = SA.Build.entity.getHeldItem(p);
      const lore = i.getLore();
...
SA.Utilities.time.setTickInterval(() => {
  for (const p of world.getPlayers()) {
    ...
    if ((!p.hasTag("mobile") || !p.hasTag("attacking") || SA.Build.entity.getHeldItem(p)?.id != "we:brush")) continue ////console.warn(p.name + " id fail");
