...
SA.Build.chat
const shovelCMD = new SA.Command({
  name: "shovel",
  ...
  .executes(
    (
      ctx,
      { blocks, высота, replaceBlocks, радиус /*zone, offset*/ }
    ) => {
      if (!blocks) {
      ...
      let bblocks
      blocks == 'st' ? bblocks = SA.Build.entity.getTagStartsWith(ctx.sender, 'st:') : bblocks = blocks
        brush.setLore([
          "§9Adv",
          `Blocks: ${bblocks}`,
