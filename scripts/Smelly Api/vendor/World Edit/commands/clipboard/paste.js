...
new SA.Command(
  ...
  (ctx, {rotation, mirror, includeEntites, includeBlocks, integrity, seed}) => {
    let b, e;
    if (!includeEntites && !includeBlocks) {e = false; b = true} else {
      b = includeBlocks; e = includeEntites
    }
    const command = WorldEditBuild.paste(ctx.sender, rotation, mirror, e, b, integrity, seed)
