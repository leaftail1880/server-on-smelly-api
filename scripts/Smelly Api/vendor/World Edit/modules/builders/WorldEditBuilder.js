...
export class WorldEditBuilder {
  ...
  drawSelection() {
    if (!this.drawsel) return SA.Build.chat.rcs([
    ...
    const ent1 = SA.Models.world.getEntityAtPos(this.pos1.x, this.pos1.y, this.pos1.z)
    const ent2 = SA.Models.world.getEntityAtPos(this.pos2.x, this.pos2.y, this.pos2.z)
    if (ent1.length == 0) {
      SA.Build.chat.runCommand(`event entity @e[type=f:t,name="${configuration.DRAW_SELECTION1_NAME}"] kill`)
      SA.Build.chat.runCommand(`summon f:t ${this.pos1.x} ${this.pos1.y} ${this.pos1.z} spawn "${configuration.DRAW_SELECTION1_NAME}"`)
    }
    if (ent2.length == 0) {
      SA.Build.chat.runCommand(`event entity @e[type=f:t,name="${configuration.DRAW_SELECTION2_NAME}"] kill`)
      SA.Build.chat.runCommand(`summon f:t ${this.pos2.x} ${this.pos2.y} ${this.pos2.z} spawn "${configuration.DRAW_SELECTION2_NAME}"`)
    }
    for (let ent of ent1) {
      if (ent.id == 'f:t' && ent.nameTag == configuration.DRAW_SELECTION1_NAME) break
      SA.Build.chat.runCommand(`event entity @e[type=f:t,name="${configuration.DRAW_SELECTION1_NAME}"] kill`)
      SA.Build.chat.runCommand(`summon f:t ${this.pos1.x} ${this.pos1.y} ${this.pos1.z} spawn "${configuration.DRAW_SELECTION1_NAME}"`)
      break;
    }
    for (let ent of ent2) {
      if (ent.id == 'f:t' && ent.nameTag == configuration.DRAW_SELECTION2_NAME) break
      SA.Build.chat.runCommand(`event entity @e[type=f:t,name="${configuration.DRAW_SELECTION2_NAME}"] kill`)
      SA.Build.chat.runCommand(`summon f:t ${this.pos2.x} ${this.pos2.y} ${this.pos2.z} spawn "${configuration.DRAW_SELECTION2_NAME}"`)
      break;
    }
  ...
  paste(
    ...
    try {
      ...
      player.runCommand(
        `structure load ${configuration.COPY_FILE_NAME} ~ ~ ~ ${
          String(rotation).replace('NaN', '0')
      ...
      return new Return(false, 0, {
        ...
        statusMessage: `§a► §rВставлено в ${Math.floor(player.location.x)} ${Math.floor(player.location.y)} ${Math.floor(player.location.z)}`,
      });
    } catch (error) {
      console.warn(error + error.stack);
      return new Return(true, 1, {
