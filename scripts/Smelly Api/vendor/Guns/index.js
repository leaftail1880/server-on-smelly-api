...
SA.Utilities.time.setTickInterval(() => {
  for (const pl of world.getPlayers()) {
    ...
    if (gs[item?.id?.split(":")[1]]) {
      ...
      if (c < 1) return [ pl.runCommand('scoreboard players set @s lockedtitle 3'), pl.onScreenDisplay.setActionBar("§cНет патрон"), ]
      empty = true;
    ...
    lore[0] = "§r§cReloading...";
    lore[1] =
      `§r§f${Number(SA.Build.entity.itemsCountClear(pl, "b:l", gun.maxAmmo - ostB)) +
      Number(ostB)}/${gun.maxAmmo}`.replace('NaN', gun.maxAmmo);
    lore[2] = "§r§7" + c;
    pl.runCommand(
...
SA.Utilities.time.setTickInterval(() => {
  for (const e of world.getDimension("overworld").getEntities()) {
    if (e.id != "f:t" || !e.nameTag.startsWith("с: ")) continue;
