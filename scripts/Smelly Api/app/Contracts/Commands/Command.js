...
import { wo, WorldOption } from "../../Models/Options.js";
...
world.events.beforeChat.subscribe((data) => {
  try {
    ...
    SA.Build.chat.broadcast(
      `§c[Error:] §f${data.message}\n§c${error}§f :\n${error.stack}`
    );
    data.cancel = true;
  }
});

new WorldOption("import:test", "Включает импорт тестовых команд");


export class Command {
  /**
   * Зарегистрировать команду
   * @param {Object} CommandInfo Описание
   * @param {string} CommandInfo.name Имя команды
   * @param {string} CommandInfo.description Описание команды
   * @param {string} CommandInfo.type Тип команды (wb || serv || test). По умолчанию test
   ...
   * @returns {void}
   * @example new Command({name: "", description: ""}, callback)
   */
  constructor(CommandInfo, callback = null) {
    ...
    this.type = CommandInfo.type ?? "test";
    // adds a new path to the stored global paths
    if (this.type != "test" || wo.Q("import:test")) {
      COMMAND_PATHS.push(this);
      HELP_PATHS.push(this);
      HELP_PATHS.sort((a, b) => (a.path[0] > b.path[0] ? 1 : -1));
    }
  }
  /**
  ...
  addSubCommand(SubCommandInfo, callback) {
    ...
    if (SubCommandInfo.tags)
      SubCommandInfo.tags.forEach((e) => {
